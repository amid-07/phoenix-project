...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-[#EAE6DA] font-sans overflow-x-hidden selection:bg-[#4ECDC4] selection:text-[#0F172A]">
      ...
      {/* --- NAVBAR --- */}
      <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-md bg-[#0F172A]/80 border-b border-white/5 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 cursor-pointer group">
            ...
            <span className="text-xl font-bold tracking-[0.2em] text-white group-hover:text-[#4ECDC4] transition-colors">TAFSUT</span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-[#EAE6DA]/80">
            ...
            <button onClick={() => document.getElementById('login-card').scrollIntoView({behavior:'smooth'})} className="bg-[#EAE6DA] text-[#0F172A] px-5 py-2.5 rounded-full font-bold hover:bg-white transition shadow-lg shadow-white/10 transform hover:scale-105">
              Espace Membre
            </button>
          </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-20 left-0 w-full bg-[#1E293B] border-b border-white/10 p-6 flex flex-col gap-4 shadow-2xl animate-in slide-in-from-top-5">
            ...
            <a href="#mission" onClick={() => setMobileMenuOpen(false)} className="text-lg font-medium">Notre Mission</a>
          </div>
        )}
      </nav>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-20 lg:pt-48 lg:pb-32 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
        ...
        <div className="space-y-8 animate-fade-in-up text-center lg:text-left">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-bold leading-tight tracking-tight text-white h-auto min-h-[160px] lg:min-h-0">
          ...
          <div className="pt-8 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-6 text-sm text-[#94A3B8]">
             <div className="flex -space-x-3">
               {[1,2,3,4].map(i => <div key={i} className="w-10 h-10 rounded-full border-2 border-[#0F172A] bg-gray-700"/>)}
             </div>
             <div className="flex flex-col text-left">
               <span className="font-bold text-white flex items-center gap-1">2,000+ membres <CheckCircle size={14} className="text-[#4ECDC4]"/></span>
               <span>nous font confiance.</span>
             </div>
          </div>
        </div>

        {/* Colonne Droite : Formulaire de Connexion (Glass Card) */}
        <div id="login-card" className="relative w-full max-w-md mx-auto lg:ml-auto group">
          {/* Effet de lueur arrière */}
          <div className="absolute -inset-1 bg-gradient-to-r from-[#6C63FF] to-[#4ECDC4] rounded-3xl blur-lg opacity-40 group-hover:opacity-60 transition duration-1000 animate-pulse-slow"></div>
          
          <div className="relative bg-[#1E293B]/80 backdrop-blur-2xl border border-white/10 p-8 rounded-2xl shadow-2xl transform transition-transform duration-300 hover:-translate-y-1">
            <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6">
               <div>
                  <h2 className="text-2xl font-bold text-white">Espace Membre</h2>
                  <p className="text-[#94A3B8] text-xs mt-1">Connectez-vous à votre dashboard</p>
               </div>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="group/input">
                <label className="text-xs font-bold text-[#94A3B8] uppercase tracking-wider mb-2 block group-focus-within/input:text-[#4ECDC4] transition">Email</label>
                <div className="relative">
                  <Mail size={18} className="absolute left-4 top-4 text-[#94A3B8] group-focus-within/input:text-white transition"/>
                  <input 
                    type="email" 
                    className="w-full bg-[#0F172A]/50 border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-[#4ECDC4] focus:ring-1 focus:ring-[#4ECDC4] transition outline-none placeholder-[#94A3B8]/30"
              ...
              <div className="group/input">
                <label className="text-xs font-bold text-[#94A3B8] uppercase tracking-wider mb-2 block group-focus-within/input:text-[#4ECDC4] transition">Mot de passe</label>
                <div className="relative">
                  <Lock size={18} className="absolute left-4 top-4 text-[#94A3B8] group-focus-within/input:text-white transition"/>
                  <input 
                    type="password" 
                    className="w-full bg-[#0F172A]/50 border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-[#4ECDC4] focus:ring-1 focus:ring-[#4ECDC4] transition outline-none placeholder-[#94A3B8]/30"
                    placeholder="••••••••"
              ...
              <button 
                disabled={loading}
                className="w-full bg-[#EAE6DA] hover:bg-white text-[#0F172A] font-bold py-4 rounded-xl transition shadow-lg shadow-white/10 flex justify-center items-center gap-2 transform active:scale-[0.98]"
              >
      ...
      <section id="features" className="max-w-7xl mx-auto px-6 py-24 border-t border-white/5">
        <div className="text-center mb-16">
          ...
          <p className="text-[#94A3B8] max-w-2xl mx-auto">Conçu par des experts en addictologie et des ingénieurs pour vous offrir le meilleur soutien possible.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <FeatureCard 
            ...
            desc="Vos données sont chiffrées de bout en bout. Votre vie privée est notre priorité."
            color="#4ECDC4"
          />
          <FeatureCard 
            icon={<Users size={32}/>} 
            title="Experts Humains" 
            desc="Prenez rendez-vous en un clic avec des psychologues certifiés."
            color="#FFD93D"
          />
          <FeatureCard 
            icon={<Heart size={32}/>} 
      ...
      <footer className="py-12 bg-[#0B1120] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <p className="text-[#94A3B8] text-sm">© 2025 TAFSUT Inc. Tous droits réservés.</p>
        </div>
      </footer>
    </div>
  );
