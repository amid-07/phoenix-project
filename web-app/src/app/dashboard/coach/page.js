...
export default function CoachDashboardPage() {
  ...
  // ⚠️ URL (Localhost pour Web local)
  ...
  const fetchData = async () => {
    try {
      ...
      const sorted = dataReq.sort((a, b) => {
        // Tri intelligent : PENDING en premier
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto text-[#EAE6DA]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <button 
          ...
          className="w-full md:w-auto bg-[#4ECDC4] hover:bg-white text-[#2F3A4A] px-6 py-3 rounded-xl font-bold flex justify-center items-center gap-2 shadow-lg transition"
        >
          <QrCode size={20} /> Valider un Billet
        </button>
      </div>
      ...
      <div className="flex bg-[#2F3A4A] p-1 rounded-xl border border-white/10 mb-8 w-full md:w-fit overflow-hidden">
         ...
         <button onClick={() => setActiveTab('planning')} className={`flex-1 md:flex-none px-6 py-2 rounded-lg font-bold transition ${activeTab === 'planning' ? 'bg-[#59647A] text-white shadow' : 'text-gray-400'}`}>Agenda</button>
      </div>
      ...
      {activeTab === 'requests' ? (
            ...
            return (
              <div key={req.id} className="bg-[#59647A] p-5 rounded-2xl border border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-lg">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                  <div className="w-12 h-12 bg-[#2F3A4A] rounded-full flex shrink-0 items-center justify-center font-bold text-lg text-[#EAE6DA]">
                ...
                <div className="flex gap-3 w-full md:w-auto justify-end">
                  {req.status === 'PENDING' && (
                    <>
                      <button onClick={() => handleAction(req.id, 'CONFIRMED')} className="bg-[#4ECDC4] text-[#2F3A4A] p-2 rounded-lg hover:bg-white transition flex-1 md:flex-none justify-center flex"><Check/></button>
                      <button onClick={() => handleAction(req.id, 'CANCELLED')} className="bg-[#FF6B6B]/20 text-[#FF6B6B] p-2 rounded-lg hover:bg-[#FF6B6B]/40 transition flex-1 md:flex-none justify-center flex"><X/></button>
                    </>
                  )}
                  {req.status === 'CONFIRMED' && (
                     req.type === 'REMOTE' ? (
                       isActive ? 
                       <button onClick={() => openVideo(req.id)} className="bg-[#6C63FF] text-white px-4 py-2 rounded-lg font-bold flex gap-2 animate-pulse w-full md:w-auto justify-center"><Video size={18}/> Lancer</button> 
                  ...
                  {req.status === 'CANCELLED' && <span className="text-[#EAE6DA]/30 text-sm">Refusé</span>}
                </div>
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           ...
           <div className="bg-[#59647A] p-6 rounded-2xl border border-white/10 h-fit">
              ...
              <input type="datetime-local" className="w-full bg-[#2F3A4A] p-3 rounded-lg text-[#EAE6DA] mb-4 border border-white/10 outline-none focus:border-[#4ECDC4]" onChange={(e) => setNewSlotDate(e.target.value)} />
      ...
      {showScanner && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4 backdrop-blur-sm" onClick={() => setShowScanner(false)}>
          <div className="bg-[#2B2B40] p-6 rounded-2xl w-full max-w-md border border-white/10 relative" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-center text-sm text-[#EAE6DA]/60 mb-2">Ou entrez le code manuellement</p>
